...
| 4.1d | **Several aggregates in one fold** · the last 1 of 8 | `SELECT delegator, sp, SUM(tok), SUM(sh) ... GROUP BY 1, 2`. Unlike the composite key, this **does** change the aggregate: `Entry` carries one `i128` and would need m. That is a different operator and a real regression risk to a gate that passes, so it is its own item rather than a relaxation |
| 4.1e | Statement-level coverage: CTEs and joins | 41 of 65 statement refusals are "projects exactly the key and the sum; got N items", N from 3 to 13, and 2 of the 7 remaining folds. Real views carry several aggregates and passthrough columns |
