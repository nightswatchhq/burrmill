...
| 6.3 | `CheckedArithmetic`, hardened from the investigation 03 prototype **· LANDED 2026-09-24; memory gate FAILS, on stock DataFusion first** | per-type accumulator, whitelist rule, wide literals refused, `TRY_CAST` tainted and its sums made exact through unions and negation; 03's table all exact-or-refused (`tests/df_checked.rs`). 989,690 groups, 8 threads, MacBook: stock DataFusion **590 MB**, checked `CAST` 606-647, `TRY_CAST` 731-745, latency within 7%. A 192 MB fair spill pool reaches 339/360 MB at 2.2-2.8x. The 256 MB gate needs the aggregate's memory owned, not the arithmetic changed. Re-measure on the thinkpad |
...
| 6.6 | `FoldSubstitution` **· LANDED 2026-09-24, ahead of 6.4/6.5; gate passed on neither machine yet** | Owned fold swapped in by an analyzer rule wherever its answer provably equals DataFusion's (NULL-rejecting filter or non-null values, strict `TRY_CAST`, 38-digit check), keys with literal tags and case folds, visible in EXPLAIN, parity by digest. 989,690 groups, streamed through `Engine::sql_for_each`: **308-314 MB, 118 ms** on the MacBook, against the bare fold's 292 MB/97 ms there and stock DataFusion's 590 MB/100 ms. The bare fold is 210 MB on the thinkpad; the hosted figure there is owed |
