...
| 5.4 | Why Burrmill is still 0.65x DuckDB at 16+ clients | Fairness, tail and liveness are all now at or better than DuckDB's; raw throughput under load is not. Not diagnosed. The obvious suspects are per-query fixed cost (footer parsing repeated per query, no plan or metadata cache) and that a narrow query still pays the full morsel-scheduling setup |
