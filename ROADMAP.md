...
| ~~6.2~~ | ~~Lockdown~~ **· LANDED with 6.1, census allowlist still open** | Non-SELECT refused at the surface. Tests for COPY, `CREATE EXTERNAL TABLE`, `read_csv`, replacement scan `FROM '/etc/passwd'`. No table factories, no URL tables. Function list is still "all default minus crypto/nested", not yet the census whitelist |
