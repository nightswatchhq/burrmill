...
| 6.5 | Dialect layer and the silent differences **· MOSTLY LANDED 2026-09-24** | DuckDB-dialect parse plus AST rewrites (`HUGEINT`, `UBIGINT`, `//`, tuple comparisons, `NOT`), `/` as DOUBLE, `to_timestamp` in µs UTC, DuckDB's text comparison rules, exact `TRY_CAST` reading, and DuckDB's default column names. `dialect-parity` **36/36** (31 identical, 5 refused by both). Open: case-insensitive quoted identifiers, `information_schema` |
