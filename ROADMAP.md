...
| 5.3a | **Adaptive per-query parallelism** · the throughput half | The gate fixed fairness and not throughput: Burrmill is 89 qps at 32 clients against `duck_multi`'s 170. Sweeping admission width 1→32 moves it only 72→98 while the tail quadruples, so **width is not the lever**. DuckDB stops parallelising a query when others are waiting; we split every query across the whole pool regardless. The fix is a `degree` on the fold — `coalesce` producing exactly `d` groups where `d = pool / in_flight` — which caps a query's width instead of merely limiting how many start |
