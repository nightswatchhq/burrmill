...
const CORPUS: &[&str] = &[
    ...
    "SELECT \"from\", arg_max(block_number, log_index) AS a, arg_min(\"to\", block_number) AS b, max_by(value, block_number) AS c, min_by(value, block_number) AS d FROM transfer GROUP BY 1 ORDER BY 1",
